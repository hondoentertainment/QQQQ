...
// A live refresh that prices fewer than this share of its holdings is treated
// as "degraded" — the holdings are fresh but most quotes failed.
export const QUOTE_OK_RATIO = 0.8;

/**
 * Summarise a refresh run into the document persisted at
 * `data/refresh-status.json` and surfaced in the dashboard footer. Pure: every
 * input is passed in, so it is unit-testable and never touches the network.
 *
 * `health` grades the run from worst to best:
 *   - `failed`   — the run could not produce holdings at all (`ok: false`).
 *   - `stale`    — no live holdings provider was reached; serving cached/seed.
 *   - `degraded` — holdings are live but fewer than `QUOTE_OK_RATIO` got priced.
 *   - `ok`       — live holdings and most quotes succeeded.
 */
export function buildRefreshStatus({
  ...
  const count = Number.isFinite(holdingsCount) && holdingsCount > 0 ? holdingsCount : 0;
  const priced = Number.isFinite(pricedCount) && pricedCount > 0
    ? Math.min(pricedCount, count) : 0;
  const quoteSuccess = count ? +(priced / count).toFixed(3) : 0;
  const fellBack = isFallbackSource(source);
  let health;
  if (!ok) health = 'failed';
  else if (fellBack) health = 'stale';
  else if (quoteSuccess < QUOTE_OK_RATIO) health = 'degraded';
  else health = 'ok';
  return {
