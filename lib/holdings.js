...
/** Build a normalized company-name → ticker map from a prior snapshot. */
export function buildNameTickerMap(holdings) {
  const map = new Map();
 ...
 * `nameToTicker`, typically built from the last good snapshot.
 */
export function parseSecNportHoldings(xml, nameToTicker = new Map()) {
  ...
  for (const m of String(xml).matchAll(/<invstOrSec>([\s\S]*?)<\/invstOrSec>/g)) {
    ...
    const ticker = lookupTickerByName(name, nameToTicker);
