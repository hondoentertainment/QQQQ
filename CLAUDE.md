...
- **Committed data is the source of truth.** Every data source is best-effort.
  The fetch script falls back Invesco -> FMP -> Slickcharts -> SEC N-PORT ->
  last-good-data so the cron job stays green and the site keeps working off the
  last snapshot. Preserve this.
